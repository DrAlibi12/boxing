...
stun = 0;
guardStun = 0; 		// KOUT-78 - GUARD STUN
dizzy = false;
dodgeFailed = false;
...
// SPECIAL CONTROL VARIABLES USED FOR SCENES
...
function initFighter(_playerId, _fighterId, _opponent=noone, _isCPU=false, _idCPU=-1) {
	...
	// HEALTH
	hp = character.maxHp;
	var _healthbarX = (playerId == 1) ? HEALTHBAR_P1_X : HEALTHBAR_P2_X;
	healthbar = instance_create_layer(_healthbarX, HEALTHBAR_Y, "GUI", obj_healthbar);
	healthbar.depth = -100;
	healthbar.init(hp, image_xscale);
	healthbar.barName = string_upper(character.name);
	
	// SET GUARD & HEALTHBAR MAX VALUES
	// GUARD
	guardHp = character.maxGuardHp;
	guardHealthbar = instance_create_layer(_healthbarX, GUARD_HEALTHBAR_Y, "GUI", obj_healthbar);
	guardHealthbar.depth = -100;
	guardHealthbar.sections = character.guardHpSections;
	guardHealthbar.sprite_index = spr_guard_healthbar;
	guardHealthbar.barHeight = sprite_get_height(spr_guard_healthbar);
	guardHealthbar.init(guardHp, image_xscale, DODGE_COST);
...
function doDodge() {
	...
	dodgeFailed = false;
	// KOUT-78 - (Temporarily) remove dodge's stamina cost & restriction
	// {
	/*if (guardHp < DODGE_COST) {
		audio_play_sound(snd_grunt_dodge_fail, 100, 0);
		shaker.shake(10, 0.5);
		dodgeFailed = true;
	} else {*/
	// } KOUT-78
...
function checkSweat() {
	// { KOUT-78 - Temporary remove the sweat particles
	//return;
	// } KOUT-78
	
	// KOUT-78 - Sweat before being punched and hurt (DODGE_COST * 1.15)
	if (hp == 0 || guardHp > DODGE_COST * 1.15 || winner) {
		dropParticles.stop();
		return;
	}
	if (dropParticles.emiting) {
		// KOUT-78 - disassociate low stamina from shakiness
		//if (!dizzy) {
		//	shaker.shake(3, 0.5);
		//}
	...
	var _quantity = (guardHp <= DODGE_COST / 2) ? 2 : 1;
	
	// START SWEAT PARTICLES 
	dropParticles.start(_quantity);
	shaker.shake(3, 0.5);
}

function checkAttackCharge() {
	// KOUT-78 - (Temporarily) remove attack's stamina cost
	//if (guardHp >= DODGE_COST && (attackInput.isPressed() || attackInput.isHeld())) {
	if (attackInput.isPressed() || attackInput.isHeld()) {
...
function checkRelease(includeDodge=true) {
	// KOUT-78 - prevent dodging after guarding
	if (includeDodge && guardStun == 0 && dodgeInput.isReleased()) {
