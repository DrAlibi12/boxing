...
if (!selected) {
	...
	for (var _i=0; _i <= optionQty - 1; _i++) {
		...
		if (_id == optionId) {
			switch (optionId) {
				case 0:
					var _textYes = getText(global.UI_TEXTS, "yes");
					var _textNo = getText(global.UI_TEXTS, "no");
					var _fullscreenText = (global.fullscreen) ? _textYes : _textNo;
					_selectedOptionText = getText(global.UI_TEXTS, options[_id, 0]) + ": " + _fullscreenText;
				...
				break;
				case 1:
					var _volumeText = string(round(global.masterVolume * 10));
					_selectedOptionText = getText(global.UI_TEXTS, options[_id, 0]) + ": " + _volumeText;
					draw_set_color(_color);
					draw_text_transformed(
						_x, _y,
						_selectedOptionText,
				...
				case 2: 
					...
					//_y += sprite_get_width(spr_flags) / 2;
					_selectedOptionText = "LANGUAGE";
					draw_sprite_ext(
