...
switch (status) {
	...
	case FighterStatus.idle:
		...
		if (hp <= character.maxHp * 0.5) {
			var _useSpecial = random(1000) < 20;
			if (_useSpecial) {
				attackInput.reset();
				dodgeInput.reset();
				specialUsed = true;
				status = FighterStatus.bossWalkBack;
			}
		}
	...
	case FighterStatus.dodge:
		...
		depth = 200;
		image_index = 12;
		// ONLY IF DODGE FAILED PLAYER GETS INVULNERABLE
		if (!dodgeFailed) {
			image_index = 3;
			invulnerable = true;
		}
	...
	case FighterStatus.jab:
		...
		if (alarm[0] == -1) {
			...
			hitbox.life = 1;
			hitbox.hitId = 0;
			hitbox.damage = character.jabDamage * strengthFactor;
			// JAB_GUARD_DAMAGE IS ALWAYS THE SAME
			hitbox.guardDamage = JAB_GUARD_DAMAGE;
			hitbox.points = JAB_SCORE;
			hitbox.stun = character.jabStun;
			
			alarm[0] = game_get_speed(gamespeed_fps) * character.jabDuration;
			var _ushing = getRandomAudio("snd_ushing", 2);
			if (_ushing != -1) audio_play_sound(_ushing, 100, 0);
	...
	case FighterStatus.hook:
		...
		if (alarm[0] == -1) {
			...
			hitbox.life = 1;
			hitbox.hitId = 1;
			hitbox.damage = character.hookDamage * strengthFactor;
			...
			hitbox.unstoppable = true;
			// KOUT-78 - HIGH EFFECTIVE DAMAGE FACTOR
	...
	case FighterStatus.hazard:
		...
		dodgeInput.check(!dizzy);
	break;
	case FighterStatus.bossWalkBack:
		// Retrieve to prepare special
		stretcher.hScale = 1;
		stretcher.vScale = 1;
		invulnerable = true;
		alarm[0] = -1;
		...
		// DODGE IMAGE
		image_index = 3;
		var _targetX = BOSS_WALK_BACK_X;
//		x = approach(x, _targetX, 10);
		x = lerp(x, _targetX, 0.3);
		
		if (abs(x - _targetX) <= 10) {
			x = _targetX;
		}
		...
		if (x == _targetX) status = FighterStatus.bossPrepareSpecial;
	break;
	case FighterStatus.bossPrepareSpecial:
		// Charge special
		image_index = 2;
		
		if (alarm[6] == -1) {
			alarm[6] = game_get_speed(gamespeed_fps) * 2;
			alarm[8] = game_get_speed(gamespeed_fps) * 1.8;
		}
	break;
	case FighterStatus.bossSpecial:
		// Attack or something
		var _targetX = P2_X;
//		x = approach(x, _targetX, 80);
		...
		if (x == _targetX) {
			status = FighterStatus.bossSpecialEnd;
			// Create hitbox
	...
	break;
	case FighterStatus.bossSpecialEnd:
		// Recovery from special (vulnerable)
		invulnerable = false;
		...
		if (alarm[0] == -1) {
			alarm[0] = game_get_speed(gamespeed_fps) * 1.5 //(specialDuration);
		}
	break;
}
