// New Keyboard Input 
function KeyboardInput(_key, _max_hold_time, _max_cancel_time, _min_cancelable_time=0) constructor{
	key = _key;
	...
	function check(_includeHold=false) {
		...
		if (is_held) {
			...
			if (keyboard_check_released(key) || (max_hold_time != -1 && hold_time > max_hold_time)) {
			...
			var _keyPressed = keyboard_check_pressed(key);
			var _keyHeld = keyboard_check(key); 
			if (_keyPressed	|| (_includeHold && _keyHeld)) {
				hold_time = 0;
				pressed_at = current_time;
				is_pressed = _keyPressed;
				is_held = true;
			}
		}
		//}
